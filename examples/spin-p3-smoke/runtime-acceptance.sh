...
port=${SPIN_ACCEPTANCE_PORT:-31080}; mock_port=18080
base_url="http://127.0.0.1:$port"; mock_url="http://127.0.0.1:$mock_port/"
spin_pid=0; mock_pid=0; spin_exit="not-stopped"; first_spin_pid=0; restart_spin_pid=0; first_spin_command=""; restart_spin_command=""; first_spin_exit=""; restart_spin_exit=""
...
if spin up --from spin.toml --listen "127.0.0.1:$port" --runtime-config-file "$runtime_cfg" >"$tmp/missing.stdout" 2>"$tmp/missing.stderr"; then fail "missing secret did not fail closed"; fi
...
start_spin success "$tmp/secret"; first_spin_pid=$spin_pid; curl -sS -X POST "$base_url/probe" --data-binary 'request_id=generated;value=payload-generated' -o "$tmp/generated.body" -w '%{http_code}' >"$tmp/generated.body.status" || true; [[ "$(<"$tmp/generated.body.status")" == 200 ]] || fail "server-generated request failed"; generated_id=$(awk -F= '/^request_id=/{print $2}' "$tmp/generated.body"); [[ "$generated_id" == probe-* ]] || fail "server-generated request ID missing"; grep -Fq server_id= "$tmp/generated.body" || fail "server-generated value missing"; request_file runtime-seq-1 'request_id=runtime-seq-1;value=payload-seq-1' "$tmp/seq-1.body"; request_file runtime-seq-2 'request_id=runtime-seq-2;value=payload-seq-2' "$tmp/seq-2.body"
...
grep -Fq fixtures=read-only "$tmp/seq-1.body" || fail "fixture read evidence missing"; grep -Fq denied_repo_file=spin.toml "$tmp/seq-1.body" || fail "repo denial missing"; grep -Fq BROWSER_UAT_OK "$tmp/seq-1.body" || fail "mock response missing"
...
for n in $(seq 1 20); do id="runtime-concurrent-$n"; grep -Fq "request_id=$id" "$tmp/concurrent-$n.body" || fail "concurrent id mismatch: $id"; grep -Fq "payload-concurrent-$n" "$tmp/concurrent-$n.body" || fail "concurrent value mismatch: $id"; for other in $(seq 1 20); do [[ "$other" == "$n" ]] && continue; grep -Fq "request_id=runtime-concurrent-$other" "$tmp/concurrent-$n.body" && fail "foreign request ID: $id"; grep -Fq "payload-concurrent-$other" "$tmp/concurrent-$n.body" && fail "foreign request value: $id"; done; done
...
cat "$tmp"/concurrent-*.body >"$tmp/concurrent-bodies.scan"; cat "$tmp"/*.stdout "$tmp"/*.stderr >"$tmp/spin-logs.scan"; cat "$tmp/generated.body" "$tmp/seq-1.body" "$tmp/seq-2.body" "$tmp"/concurrent-*.body "$tmp/restart.body" >"$tmp/http-bodies.scan"
scan_labels=(source diff manifest config skill artifact missing_stdout missing_stderr empty_stdout empty_stderr whitespace_stdout whitespace_stderr generated_body seq1_body seq2_body concurrent_bodies restart_body mock_capture spin_logs kv_db evidence); scan_paths=("$root" "$tmp/source-diff" "$root/spin.toml" "$root/fixtures/config.json" "$root/fixtures/SKILL.md" "$root/main.wasm" "$tmp/missing.stdout" "$tmp/missing.stderr" "$tmp/empty.stdout" "$tmp/empty.stderr" "$tmp/whitespace.stdout" "$tmp/whitespace.stderr" "$tmp/generated.body" "$tmp/seq-1.body" "$tmp/seq-2.body" "$tmp/concurrent-bodies.scan" "$tmp/restart.body" "$mock_receipts" "$tmp/spin-logs.scan" "$state_path" "$out"); scan_counts=()
