...
sentinel=$(<"$tmp/secret"); diff_base=$(git merge-base origin/main HEAD 2>/dev/null) || fail "cannot resolve merge-base for origin/main and HEAD"; [[ -n "$diff_base" ]] || fail "empty merge-base for origin/main and HEAD"; git diff --binary "$diff_base" HEAD -- examples/spin-p3-smoke PORT_STATUS.md >"$tmp/source-diff" || fail "cannot generate scoped source diff"; source_diff_hash=$(shasum -a 256 "$tmp/source-diff" | awk '{print $1}'); source_diff_bytes=$(wc -c <"$tmp/source-diff" | tr -d ' '); [[ "$source_diff_bytes" -le 2000000 ]] || fail "scoped source diff exceeded 2000000 bytes"
